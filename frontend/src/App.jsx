...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .sidebar-label { font-size: 10px; color: var(--text3); text-transform: uppercase; letter-spacing: 1.5px; padding: 0 8px 6px; font-family: var(--mono); }

  .channel-item {
    display: flex; align-items: center; gap: 8px;
    padding: 8px 10px; border-radius: 8px; cursor: pointer;
    transition: background .15s; position: relative; group: true;
  }
  .channel-item:hover { background: var(--bg3); }
  .channel-item.active { background: var(--accent)18; }
  .channel-item .ch-name { flex: 1; font-size: 13px; font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .channel-item .ch-count { font-size: 10px; color: var(--text3); font-family: var(--mono); }
  .channel-item .ch-del { opacity: 0; transition: opacity .15s; }
  .channel-item:hover .ch-del { opacity: 1; }
  .channel-item.drag-over { border-top: 2px solid var(--accent2); }
  .channel-item[draggable] { cursor: grab; }
  .ch-dot { width: 8px; height: 8px; border-radius: 50%; background: var(--accent); flex-shrink: 0; }
...
function MoveModal({ file, tree, onClose, onMoved }) {
  ...
  async function submit() {
    ...
    setLoading(true)
    try {
      await api.moveFile(file.id, selected)
      toast('Movido a ' + selected, 'success')
      onMoved()
    } catch (e) {
  ...
  return (
    <div className="lightbox" onClick={onClose}>
      <div className="lightbox-bar" onClick={e => e.stopPropagation()}>
        ...
        <button onClick={onClose} style={{ background: 'none', border: 'none', color: '#fff', fontSize: 20, cursor: 'pointer', lineHeight: 1 }}>
          x
        </button>
      </div>
...
function FileCard({ file, view, onDownload, onMove, onDelete, onPreview }) {
    ...
    return (
      <div className="file-row" draggable onDragStart={e => { e.dataTransfer.setData('teldrive-file-id', String(file.id)); e.dataTransfer.effectAllowed = 'move' }}>
        ...
        <span style={{ display: 'flex', justifyContent: 'flex-end', gap: 4 }}>
          <button className="btn btn-ghost btn-icon" onClick={() => onMove(file)} title="Mover">
            <Icon d={icons.move} size={14} />
          </button>
          <button className="btn btn-ghost btn-icon" onClick={() => onDownload(file)} title="Descargar">
            <Icon d={icons.download} size={14} />
          </button>
          <button className="btn btn-ghost btn-icon" onClick={() => onDelete(file)} title="Eliminar" style={{ color: 'var(--danger, #ef4444)' }}>
  ...
  return (
    <div className="file-card" style={{ '--type-color': color }} draggable onDragStart={e => { e.dataTransfer.setData('teldrive-file-id', String(file.id)); e.dataTransfer.effectAllowed = 'move' }}>
      {(file.type === 'image' || file.type === 'video')
        ? <div style={{ cursor: file.type === 'image' ? 'zoom-in' : 'default' }} onClick={() => onPreview && onPreview(file)}><LazyThumb fileId={file.id} icon={getFileIcon(file.type)} /></div>
      ...
      <div className="file-actions">
        <button className="btn btn-ghost btn-icon" style={{ fontSize: 12 }} onClick={() => onMove(file)} title="Mover">
          <Icon d={icons.move} size={13} />
        </button>
        <button className="btn btn-ghost btn-icon" style={{ fontSize: 12 }} onClick={() => onDownload(file)} title="Descargar">
          <Icon d={icons.download} size={13} />
        </button>
        <button className="btn btn-ghost btn-icon" style={{ fontSize: 12, color: 'var(--danger, #ef4444)' }} onClick={() => onDelete(file)} title="Eliminar">
      ...
      <div className="app">
        ...
        <aside className="sidebar">
          <div className="sidebar-header">
            <div className="logo">Tel<span>Drive</span></div>
            <div className="logo-sub">tu cloud en telegram</div>
          </div>
          ...
          <div className="sidebar-section">
            ...
            {channels.map((ch, idx) => (
              <div key={ch.id}
                className={'channel-item' + (activeChannel && activeChannel.id === ch.id ? ' active' : '') + (chDragOver === idx && chDragIdx !== idx ? ' drag-over' : '')}
                draggable
                ...
                <div className="ch-dot" style={{ background: activeChannel && activeChannel.id === ch.id ? 'var(--accent2)' : 'var(--border2)', cursor: 'grab' }} />
                <span className="ch-name">{ch.name}</span>
                <span className="ch-count">{ch.file_count}</span>
                <button className="btn btn-icon ch-del" style={{ ...ICON_BTN, color: 'var(--accent2)' }}
                  title='Sincronizar' onClick={e => handleSync(ch, e)}>
                  {syncing[ch.id] ? <span className="spinner" style={{ width: 12, height: 12 }} /> : <Icon d={icons.sync} size={13} />}
                </button>
                <button className="btn btn-icon ch-del" style={{ ...ICON_BTN, color: 'var(--text3)' }}
                  title="Re-indexar" onClick={e => handleIndex(ch, e)}>
                  {indexing[ch.id] ? <span className="spinner" style={{ width: 12, height: 12 }} /> : <Icon d={icons.refresh} size={13} />}
                </button>
                {channels.length > 1 && (
                  <button className="btn btn-icon ch-del"
                    style={{ ...ICON_BTN, color: defaultChannelId === ch.id ? '#f59e0b' : 'var(--text3)' }}
                    title={defaultChannelId === ch.id ? 'Canal de inicio (click para quitar)' : 'Marcar como canal de inicio'}
                    onClick={e => {
                      e.stopPropagation()
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 12, minWidth: 220 }}>
                    {channels.map(ch => (
                      <button key={ch.id} className="btn" style={{ justifyContent: 'space-between', padding: '10px 16px', background: 'var(--bg2)', border: '1px solid var(--border2)', fontSize: 14 }}
                        onClick={() => selectChannel(ch)}>
                        <span>📺 {ch.name}</span>
                        <span style={{ fontSize: 11, color: 'var(--text3)' }}>{ch.file_count} archivos</span>
                      </button>
                    ))}
                  </div>
                </div>
